...
export class DriverDetailGeneralInfoComponent implements OnInit, OnDestroy {
      ...
      mergeMap(selectedBusiness => {
        ...
        .pipe(
          mergeMap(ok => {
            ...
            return this.DriverDetailService.createServiceDriver$(this.driver);
          }),
    ...
    this.showConfirmationDialog$("SERVICE.UPDATE_MESSAGE", "SERVICE.UPDATE_TITLE")
      .pipe(
        mergeMap(ok => {
          ...
          return this.DriverDetailService.updateServiceDriverGeneralInfo$(this.driver._id, generalInfoinput);
        }),
        mergeMap(resp => this.graphQlAlarmsErrorHandler$(resp)),
        filter((resp: any) => !resp.errors || resp.errors.length === 0),
        takeUntil(this.ngUnsubscribe)
  ...
  onDriverStateChange() {
    this.showConfirmationDialog$("SERVICE.UPDATE_MESSAGE", "SERVICE.UPDATE_TITLE")
      .pipe(
        mergeMap(ok => {        
          return this.DriverDetailService.updateServiceDriverState$(this.driver._id, this.driverStateForm.getRawValue().state);
        }),
        mergeMap(resp => this.graphQlAlarmsErrorHandler$(resp)),
        filter((resp: any) => !resp.errors || resp.errors.length === 0),
        takeUntil(this.ngUnsubscribe)
