...
class MongoDB {
  ...
  createIndexesOnHistoricalCollection$() {
    return Observable.create(async observer => {
      ...
      await historicalDB.collection("Service").createIndex({businessId: 1, closed: 1, state: 1, "client.username": 1, "client.fullname": 1, "vehicle.licensePlate": 1, "driver.fullname": 1, "driver.documentId": 1, timestamp: 1}, { background: true,  name: "businessId_1_closed_1_state_1_client.username_1_client.fullname_1" })
        .catch((err) => console.log(`Failed to create index: ${err}`));
