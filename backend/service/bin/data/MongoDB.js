...
class MongoDB {
  ...
  createIndexesOnHistoricalCollection$() {
    return Observable.create(async observer => {
      ...
      observer.next(`Creating index for ${historicalDB.databaseName}.Service => ({ businessId: 1, closed: 1, state: 1, request.sourceChannel: 1, timestamp: 1 })`);
      await historicalDB.collection("Service").createIndex({ businessId: 1, closed: 1, state: 1, "request.sourceChannel": 1, timestamp: 1 })
        .catch((err) => console.log(`Failed to create index: ${err}`));

      observer.next(`Creating index for ${historicalDB.databaseName}.Service => ({ businessId: 1, closed: 1, state: 1, timestamp: 1 })`);
      await historicalDB.collection("Service").createIndex({ businessId: 1, closed: 1, state: 1, timestamp: 1 })
        .catch((err) => console.log(`Failed to create index: ${err}`));
