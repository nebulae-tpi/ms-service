...
const VALID_SERVICE_PAYMENT_TYPES = ['CASH', 'NEQUI', 'DAVIPLATA', 'BANCOLOMBIA', 'CREDIT_CARD'];
const VALID_SERVICE_REQUEST_FEATURES = ['AC', 'TRUNK', 'ROOF_RACK', 'PETS', 'VIP', 'JUMPER_CABLES', 'BIKE_RACK'];
 ...
 */
  ...
  acceptServiceOffer$({ root, args, jwt }, authToken) {
    ...
    return RoleValidator.checkPermissions$(authToken.realm_access.roles, "service-core.ServiceCQRS", "acceptServiceOffer", PERMISSION_DENIED, ["DRIVER"]).pipe(
      ...
      mergeMap(shift => ServiceDA.assignService$(serviceId, shift._id, shift.driver, shift.vehicle, location, { shiftId: 1, vehicle: 1, driver: 1, paymentType: 1, location: 1, _id: 0 })),
